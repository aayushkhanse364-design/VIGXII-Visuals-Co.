...
function isAllowedAdminEmail(email: string) {
  const allowed = (import.meta.env.VITE_ADMIN_EMAILS || import.meta.env.VITE_FIREBASE_ADMIN_EMAILS || '')
    .split(',')
...
export default function App() {
  ...
  async function handleFileUpload(file: File, kind: 'image' | 'video') {
    ...
    try {
      ...
      if (result.resourceType !== kind) {
        toast.message('Uploaded successfully, but the media type differs from the selected field');
      }
  ...
  if (authLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center px-6 text-white/70">
        <Loader2 className="mr-3 animate-spin" size={18} /> Checking session...
      </div>
    );
  }
        ...
        <section className="mx-auto grid max-w-6xl gap-10 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="space-y-8 rounded-[2rem] border border-white/10 bg-white/5 p-8 shadow-[0_24px_120px_rgba(0,0,0,0.35)] backdrop-blur-xl lg:p-12">
            ...
            <p className="max-w-2xl text-base leading-8 text-white/70 md:text-lg">
              Firebase email/password login, backend token verification, and Cloudinary media upload keep content publishing secure and simple.
            </p>
          ...
          <form
            className="rounded-[2rem] border border-white/10 bg-black/30 p-8 shadow-[0_24px_120px_rgba(0,0,0,0.35)] backdrop-blur-xl lg:p-12"
          ...
          >
            ...
            <div className="mt-8 space-y-5">
              <label className="block space-y-2 text-sm text-white/70">
                ...
                <input
                  type="email"
                  className="w-full rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white outline-none transition focus:border-[#c8a96a]/60"
            ...
            <button
              type="submit"
              className="mt-8 inline-flex w-full items-center justify-center gap-2 rounded-full bg-[#c8a96a] px-5 py-3 text-sm font-medium text-black transition hover:bg-[#e2c98f]"
            >
      ...
      <section className="mx-auto max-w-7xl space-y-8">
        <header className="flex flex-col gap-4 rounded-[2rem] border border-white/10 bg-white/5 p-6 shadow-[0_24px_120px_rgba(0,0,0,0.35)] backdrop-blur-xl md:flex-row md:items-center md:justify-between md:p-8">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-4 py-3 text-sm text-white/75 transition hover:border-[#c8a96a]/60 hover:text-[#c8a96a]"
          >
        ...
        <section className="grid gap-8 xl:grid-cols-[0.95fr_1.05fr]">
          <form onSubmit={handleSubmit(onSubmit)} className="rounded-[2rem] border border-white/10 bg-black/30 p-6 shadow-[0_24px_120px_rgba(0,0,0,0.35)] backdrop-blur-xl md:p-8">
            <div className="flex items-center justify-between gap-3">
              ...
              {editingProjectId ? (
                <button
                  ...
                  className="rounded-full border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/70 transition hover:border-white/20"
                >
                  Cancel
                </button>
              ) : null}
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              <button
                ...
                className="ml-auto inline-flex items-center gap-2 rounded-full bg-[#c8a96a] px-5 py-3 text-sm font-medium text-black transition hover:bg-[#e2c98f] disabled:cursor-not-allowed disabled:opacity-60"
              >
          ...
          <div className="rounded-[2rem] border border-white/10 bg-black/30 p-6 shadow-[0_24px_120px_rgba(0,0,0,0.35)] backdrop-blur-xl md:p-8">
            ...
            <div className="mt-8 space-y-4">
              {projectsLoading ? (
                <div className="flex items-center gap-3 rounded-3xl border border-white/10 bg-white/5 p-5 text-white/60">
                    ...
                    <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                      <div className="space-y-3">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {project.githubLink ? (
                            <a href={project.githubLink} target="_blank" rel="noreferrer" className="text-white/45 transition hover:text-[#c8a96a]" aria-label="Open GitHub link">
                              <Github size={16} />
                            </a>
                          ) : null}
                        </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {project.techStack.map((tech) => (
                            <span key={tech} className="rounded-full border border-white/10 bg-black/20 px-3 py-1 text-[11px] uppercase tracking-[0.2em] text-white/55">
                              {tech}
                            </span>
                          ))}
                        </div>
                      ...
                      <div className="flex gap-2">
                        ...
                        <button
                          ...
                          className="inline-flex h-10 w-10 items-center justify-center rounded-full border border-white/10 bg-white/5 text-white/70 transition hover:border-red-400/60 hover:text-red-300 disabled:cursor-not-allowed disabled:opacity-50"
                ...
                <div className="rounded-3xl border border-dashed border-white/15 bg-white/5 p-8 text-center text-white/55">
